    ...
    <div className="p-2 pr-4 pl-0">
      ...
      <div className="flex flex-wrap justify-between pt-3 px-2 gap-3">
        {instructors.map((instructor, index) => (
          <a key={index} className="no-underline" href={instructor.link} rel='noopener' target='_blank'>
            <div className="">
              <img src={instructor.image} alt={instructor.name} className="w-20 aspect-square object-cover rounded-full border-4 border-solid border-gray-500" />
            </div>
            <p className="text-sm my-2 truncate max-w-20">{instructor.name}</p>
          </a>
        ))}
      </div>
    </div>
